...
void kDistant(Node* node, int path[], bool visited[],
                          int pathLen, int k,int &c)
{
    ...
    {
        cout << path[pathLen - k - 1] << " ";           //print the nodes
        c++;
        visited[pathLen - k - 1] = true;
        return;
    }
    kDistant(node->left, path, visited, pathLen, k,c);
    kDistant(node->right, path, visited, pathLen, k,c);
}
...
int printKDistantfromLeaf(Node* root, int k)
{
    ...
    int c=0;                                             //store number of nodes
    kDistant(root, path, visited, 0, k,c);
    return c;
}